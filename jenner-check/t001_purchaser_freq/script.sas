/* From Electoral_Bonds_Analysis.sas (lines 7-9): frequency of bond purchases
   per purchasing company.

   The upstream script reads mydata from Purchaser_Comp_All_Cols.xlsx via a
   SAS Studio home path (/home/u63714413/Riya/...). So this bundle is fully
   self-contained, WORK.mydata is rebuilt here from a small representative
   sample of the real purchaser workbook (same columns the author reads), and
   the author's PROC FREQ then runs against it unmodified. */
data mydata;
